...
int max_buts(int size_joueurs) {
    ...
    for (int i = 1; i < size_joueurs; i++) {
        if (equipe[i].buts > max && equipe[i].id != 0) {
            max = equipe[i].buts;
        }
    }
...
void afficher_joueurs_marque_xbuts(int size_joueurs) {
    ...
    int x_buts, joueur_existe = 0, max = max_buts(size_joueurs);
    ...
    if (!joueur_existe && x_buts > max) {
        printf("\nLe nombre de buts saisi ne doit pas depasser le maximum marque par un joueur!\n");
