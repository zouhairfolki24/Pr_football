...
int main() {
    ...
    while (size_integers > 1) { // size_integers = 2
        ...
        pos_min = position_minimum(integers, size_integers); // ({42, 63, 17}, 2)
        
        value_temp = integers[size_integers - 1];// 63
        integers[size_integers - 1] = integers[pos_min]; // {42, 42, 17}
        integers[pos_min] = value_temp; // {63, 42, 17}
