...
void ajouter_plusieurs_joueur() {
    ...
    struct Joueur joueur;
    int index = 0;
    int nombre_joueurs = get_nombre_joueurs(100);
...
int main() {
    ...
    ajouter_plusieurs_joueur();

    for (int i = 0; i < 100; i++) {
        printf("\nID: %d, Nom: %s, Prenom: %s, Numero Maillot: %d, Poste: %s, Age: %d, Buts: %d.\n",equipe[i].id, equipe[i].nom, equipe[i].prenom, joueur.numeroMaillot, joueur.poste, joueur.age, joueur.buts);
    }
