...
int recherche_par_id(int id, int size_joueurs) {

    for (int i = 0; i < size_joueurs; i++) {
        if (equipe[i].id == id && equipe[i].id != 0) {
            return i;
        }
    }
...
void modifier_poste_joueur(int size_joueurs) {
    ...
    pos_joueur = recherche_par_id(new_id, size_joueurs);
...
void modifier_buts_joueur() {

    int pos_joueur;

    pos_joueur = recherche_par_id(i)
...
int main() {
    ...
    char choix[20], choix_ajoute[20], choix_affiche[20], choix_modification[20];
